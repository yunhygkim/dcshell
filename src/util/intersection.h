...
template<class real> int AreaSign(const real a[2], const real b[2], const real c[2])
{
    ...
    if( sizeof(a[0])<=sizeof(double) ){
        if( (area>0 && area<SMALLNUMBER) || (area<0 && area>-SMALLNUMBER) )
        {
            ...
            real A[2]; A[0]=a[0]; A[1]=a[1];
            real B[2]; B[0]=b[0]; B[1]=b[1];
            real C[2]; C[0]=c[0]; C[1]=c[1];
...
// (o,n) defines a plane
//
// (p1,p2) defines a segment
//    note we treat the segment as [p1,p2), i.e., the p2 end is open
//
// x is the intersection
//
// return:
//    'p' if parallel
//    'i' if p1 is on the plane (p2 is ignored if it's on the plane)
//    '1' if [p1,p2) intersects the plane
//    '0' if no intersection
//
template<class real>
char plane_segment_intersection
(const Point<real,3>& o,  const Vector<real,3>& n, //this defines the plane
 const Point<real,3>& p1, const Point<real,3>& p2, Point<real,3>& x)
{
    real t=(p1-o)*n;
    real s=(p2-o)*n;


    if(fabs(t)<SMALLNUMBER && fabs(s)<SMALLNUMBER){ x=p1; return 'p'; }
    if(fabs(t)<SMALLNUMBER){ x=p1; return 'i'; }
    if( (t>0&&s>=0) || (t<0&&s<=0) ) return '0'; //no intersection
    if(t<0) t=-t;
    if(s<0) s=-s;
...
// (o,n) defines a plane
//
// t  defines a triangle
//
// compute the intersection
// (assuming that they are not parallel)
//
// Returns:
//      0: no intersection (seg and eid are undefined)
//      v: a vertex of t is on the plane (seg[1] and eid[1] are undefined)
//      s: regular intersection
//

template<class real> char tri_plane_intersection
(const Point<real,3>& o, const Vector<real,3>& n, Point<real,3> t[3],
 Point<real,3> seg[2], short eid[2])
{
    typedef Point<real,3> rPoint;
    list<rPoint> tmp_pt;
    list<short> tmp_id;
    ...
    for(short j=0;j<3;j++){
        ...
        if(k==3) k=0;
        rPoint x;
        char r=plane_segment_intersection(o,n,t[j],t[k],x);
        if(r=='0') continue; //no intersection
        if(r=='p'){ //this segment is parallel to f
            tmp_pt.clear(); //may contain other 'i' points
            tmp_id.clear();
            tmp_pt.push_back(t[j]);
            tmp_pt.push_back(t[k]);
            tmp_id.push_back(j);
            tmp_id.push_back(j);
            break; //done....there are no other cases
        }
        //otherwise there is one intersection
        tmp_pt.push_back(x);
        tmp_id.push_back(j); //x is from j
    }//end of j

    //this should not happen...
    if(tmp_pt.empty()){
        cerr<<"! Warning: tri_plane_intersection returns no intersection."<<endl;
        return '0';
    }

    //there more than one point
    ...
    if(tmp_pt.size()>1)
    {
      ...
      for(auto& pt:tmp_pt){
        ...
        if(isunique) unique.push_back(pt);
      }
      //done
      unique.swap(tmp_pt);
    }

    //this only happens when tmp.size==1, i.e., a vertex of d->f
    //collide with facet f, so there is no segment created
    if(tmp_pt.size()==1){
        seg[0]=tmp_pt.front();
        eid[0]=tmp_id.front();
        return 'v';
    ...
    if(tmp_pt.size()>2)
    {
        cerr<<"! Warning: tri_plane_intersection found too many intersections ("<<tmp_pt.size()<<" instead of 2)."<<endl;
        cerr<<"! plane= (o="<<o<<", n="<<n<<") and t=("<<t[0]<<","<<t[1]<<","<<t[2]<<")"<<endl;
        cerr<<"! intersection= ";
        for(auto & pt: tmp_pt) cerr<<"("<<pt<<") ";
        cout<<endl;
        cerr<<"! ids= (";
        for(auto & id: tmp_id) cerr<<id<<" ";
        cout<<")"<<endl;
    }
    ...
    return 's'; //a segment is created without errors
}

//
// triangle&triangle intersection
//
// given two triangles t1 and t2 and their normals
// compute the intersection
// (assuming that they are not parallel)
//
// Return:
//
//    0: if these is no proper intersection
//       (it's possible that there is degenreated intersection)
//    s: regular intersection
//
//
template<class real>
char tri_tri_intersection
(Point<real,3> t1[3], const Vector<real,3>& n1, //first triangle
 Point<real,3> t2[3], const Vector<real,3>& n2, //second triangle
 Point<real,3> s[2],  // the segment
 bool e1x[6], bool e2x[6])  // edges contributing to the segment
{
    typedef Point<real,3>  rPoint;
    typedef Vector<real,3> rVector;

    rPoint s1[2], s2[2]; //segment
    short  e1[2], e2[2]; //contributing edge id

    char r1=tri_plane_intersection(t2[0],n2,t1,s1,e1);
    if(r1!='s') return '0';

    char r2=tri_plane_intersection(t1[0],n1,t2,s2,e2);
    if(r2!='s') return '0';

    //compute the intersection of s1 and s2
    rVector v=n1%n2; //(s1[1]-s1[0]);
    //real v_norm=v.norm();
    //v=v/v_norm;
    real i1[2]={0,(s1[1]-s1[0])*v};
    real i2[2]={(s2[0]-s1[0])*v, (s2[1]-s1[0])*v };
    if(i1[0]>i1[1]){ swap(i1[0],i1[1]); swap(e1[0],e1[1]); } //make sure i1[0] is smaller
    if(i2[0]>i2[1]){ swap(i2[0],i2[1]); swap(e2[0],e2[1]); } //make sure i2[0] is smaller

    if(i1[0]>=i2[1] || i1[1]<=i2[0]) return '0'; //no actual overlap..

    //compute the positions of the end points
    real& m=(i1[0]>i2[0])?i1[0]:i2[0]; //max
    real& n=(i1[1]<i2[1])?i1[1]:i2[1]; //min

    v=v/v.normsqr();
    s[0]=s1[0]+(v*m);
    s[1]=s1[0]+(v*n);

    //find contributing edges (edges that create this segment)
    for(short i=0;i<6;i++){ e1x[i]=e2x[i]=false; }

    if(m==i1[0]) e1x[e1[0]*2]=true; //m is from t1
    if(m==i2[0]) e2x[e2[0]*2]=true; //m is from t2

    if(n==i1[1]) e1x[e1[1]*2+1]=true; //n is coming from t1
    if(n==i2[1]) e2x[e2[1]*2+1]=true; //m is from t2

    //when an edge overlap with the other triangle
    if(e1[0]==e1[1]){ e1x[e1[0]*2]=e1x[e1[0]*2+1]=true; }
    if(e2[0]==e2[1]){ e2x[e2[0]*2]=e2x[e2[0]*2+1]=true; }

//    short eid1=(&m==&i1[0])?e1[0]:e2[0]+3;
//    short eid2=(&n==&i1[1])?e1[1]:e2[1]+3;
//
//    e2x[eid1]=0; //this edge contribute to the first end-point
//    e2x[eid2]=1; //this edge contribute to the second end-point


    //if(m==n) return '0'; //no actual overlap..
